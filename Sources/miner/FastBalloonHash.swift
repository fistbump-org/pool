...
enum FastBalloonHash {
    ...
    ///
    /// - Parameters:
    ///   - password: Pre-hashed password (32 bytes).
    ///   - salt: Nonce + extraNonce (28 bytes).
    ///   - buffer: Reusable mining buffer (must have matching slot count).
    static func hash(
        password: [UInt8],
        salt: [UInt8],
        buffer: MiningBuffer,
    ...
    private static func blake2b256_single(
        ...
        // 12 rounds using flat sigma array
        for r in 0..<12 {
            let base = r * 16
        ...
        // Finalize (only first 4 words needed for 256-bit output)
